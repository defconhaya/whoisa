...
async fn ip(semaphore: Arc<Semaphore>, ip: String, progress_bar: Arc<ProgressBar>) -> Result< WhoisInfo, String> {
...
async fn main() {
    ...
    match File::open(&args.file) {
        Ok(file_handle) => {
            ...
            let line_count = fs::read_to_string(args.file)
            ...
            for line in reader.lines() {
                match line {
                    Ok(line_content) => {
                        ...
                        people_handlers.push(tokio::spawn(ip(
                            semaphore_arc.clone(),
            ...
            for result in results {
                let keys_to_try = vec!["org-name", "organization", "orgname", "netname"];
